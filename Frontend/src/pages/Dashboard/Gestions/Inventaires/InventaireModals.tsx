...
const InventaireModals: React.FC<InventaireModalsProps> = ({ modalType, selectedItem, onClose, onSuccess }) => {
  ...
  const [terminateSuccess, setTerminateSuccess] = useState<{ show: boolean, reportUrl: string | null, pdfUrl?: string | null }>({ show: false, reportUrl: null, pdfUrl: null });
  ...
  const handleTerminate = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        setTerminateSuccess({ show: true, reportUrl: res.data.report_url, pdfUrl: res.data.pdf_report_url });
        window.open(`${BACKEND_URL}${res.data.pdf_report_url}`, '_blank');
        onSuccess(true);
      }
    } catch (err: any) {
